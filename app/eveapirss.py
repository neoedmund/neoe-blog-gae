...
implpaths=["/char/WalletJournal","/char/WalletTransactions","/char/SkillQueue","/char/IndustryJobs","/char/MarketOrders"]
def html2post(xml,path,cid):
	t=getXMLTree(xml)	
	posts=[]
	cnt=0
	if not hasattr(t.eveapi[0],"result"):
		logging.info(xml[:200])
		return []
	if path=="/char/IndustryJobs":
		...
		for r in t.eveapi[0].result[0].rowset[0].row:
			...
			p.author = "eve"
			p.title ="MarketOrders: orderID=%s,typeID=%s,vol=%s/%s" \
			    % (r.orderID, r.typeID, r.volRemaining, r.volEntered)
			if  int(r.duration<=1):
		...
		for r in t.eveapi[0].result[0].rowset[0].row:
			if path=="/char/WalletJournal":
				...
				logging.info("unkown path:%s" % path)
				break
			cnt+=1
			if cnt>20:break
